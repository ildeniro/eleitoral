...
$distribuicao = isset($_POST['distribuicao']) ? $_POST['distribuicao'] : false;
    ...
    try {
        ...
        if ($id == 0) {
            ...
            if ($distribuicao == true) {
                $sql = $db->prepare("UPDATE 2024_voluntarios SET nome = ?, celular = ?, cpf = ?, nm_pai = ?, nm_mae = ?, whatsapp = ?, endereco = ?, bsc_bairros_id = ?, numero = ?, nascimento = ?, rg = ?, orgao_expedidor = ?, deficiencia = ?, sexo = ?, treinamento = ?, treinamento_data_participacao = ?, treinamento_motivo = ?, usuario_id = ? WHERE id = ?");
                $sql->bindValue(1, $nome_fiscal);
                $sql->bindValue(2, $celular_fiscal);
                $sql->bindValue(3, $cpf_fiscal);
                $sql->bindValue(4, $pai);
                $sql->bindValue(5, $mae);
                $sql->bindValue(6, $whatsapp);
                $sql->bindValue(7, $endereco_fiscal);
                $sql->bindValue(8, $bairro);
                $sql->bindValue(9, $numero);
                $sql->bindValue(10, $nascimento);
                $sql->bindValue(11, $rg);
                $sql->bindValue(12, $orgao_emissor);
                $sql->bindValue(13, $deficiencia);
                $sql->bindValue(14, $sexo);
                $sql->bindValue(15, $treinamento);
                $sql->bindValue(16, $data_participacao);
                $sql->bindValue(17, $motivo);
